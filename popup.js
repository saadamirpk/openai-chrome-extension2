document.addEventListener('DOMContentLoaded', function () {
    ...
    const mainFunctionalitiesSection = document.getElementById('main-functionalities');
    ...
    submitApiKeyButton.addEventListener('click', function () {
        ...
        if (apiKey) {
            ...
            chrome.storage.sync.set({ 'apiKey': apiKey }, function () {
                ...
                console.log("Is floating button displayed?", floatingButton.style.display);

                // Debugging: Check if dropdown menu exists and is displayed
                console.log("Is dropdown menu displayed?", dropdownMenu.style.display);
            });
        } else {
    ...
    document.getElementById('passAndReplaceButton').addEventListener('click', function () {
        // Get the selected API, prompt, and apiKey from the popup
        const selectedApi = document.getElementById('apiOptions').value;
        const selectedPrompt = document.getElementById('promptOptions').value;
        ...
        chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
            ...
            chrome.tabs.sendMessage(tab.id, { action: "getSelectedText" }, function (response) {
                ...
                chrome.runtime.sendMessage({
                    ...
                    if (response.success) {
                        // Replace the selected text with the generated output
                        const generatedOutput = response.response.choices[0].text;
    ...
    function populatePromptOptions(selectedApi) {
        ...
        const apiPromptMap = {
            api1: ['Fix Grammar in the following sentence', 'Fix Spelling mistakes in the following sentence'],
            api2: ['Fix Punctuation in the following sentence', 'Remove Extra Spaces in the following sentence'],
            api3: ['Make the following sentence more formal', 'Make the following sentence more casual']
            ...
            if (selectedText && generatedOutput) {
                ...
                if (focusedElement) {
                    ...
                    if (focusedElement.tagName === 'INPUT' || focusedElement.tagName === 'TEXTAREA') {
                        ...
                        focusedElement.value = beforeText + generatedOutput + afterText;
                    } else {
                        ...
                        if (selection.rangeCount > 0) {
                            ...
                            range.insertNode(document.createTextNode(generatedOutput));
                        }
                    }
                }
            }
